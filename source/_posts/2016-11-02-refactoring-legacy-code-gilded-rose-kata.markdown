...
The [Gilded Rose Kata](https://github.com/emilybache/GildedRose-Refactoring-Kata) is an exercise in taking a system that calculates "quality" and expriy dates for items in a shop over time. When you start the Kata, you have a single method that calculates the changes in quality and expiry for the items. The business logic implemented is hard to understand, and features nested `if` statements with conditionals that feel inverted. You also need to add functionality to support a new item type to the shop to complete the Kata.
...
Legacy System maintenance is hard, but not impossible. You can always get a piece of business logic to be able to be tested, and restructure messy code so it's simple and understandbale. Techniques such a sprout method make gaining initial understanding and making code a bit more clear realitively easy to perform. [Working Effectively With Legacy Code](http://www.bookdepository.com/Working-Effectively-with-Legacy-Code-Michael-Feathers/9780131177055?a_aid=rickerbh) was a super-valuable resource, and covers many, many techniques to get an unweildy codebase under control, both in terms of gaining understanding as well as getting code testable.
